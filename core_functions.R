...
geneEnrich <- function(dz_signature, 
  ...
  if(length(up.genes)>0){
    ...
    names(up.gene.res)=c("database",	"category",	"qval",	"Odds_Ratio","Combined Score",	"genes")
    up.gene.res = up.gene.res[, c("database",	"category",	"Odds_Ratio", "qval",	"Combined Score",	"genes")]
  ...
  if(length(dn.genes)>0){
    ...
    names(dn.gene.res)=c("database",	"category",	"qval",	"Odds_Ratio","Combined Score",	"genes")
    dn.gene.res = dn.gene.res[, c("database",	"category",	"Odds_Ratio", "qval",	"Combined Score",	"genes")]
...
visualize_dz_sig_pathway <- function(res){
  dz_signature = res #read.csv(paste0(outputFolder,'/dz_signature.csv'),stringsAsFactors = F)
  ...
  for (database_val in databases){
    ...
    up_down_subset = up_down_subset[order(up_down_subset$qval), ]
